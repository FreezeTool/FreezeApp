...
public class IceBoxManager {
    ...
    public static void requestForceStopApp(String packageName, Callback2 callback) {
        ShellClient.command(String.format("am force-stop %s", packageName), new ShellClient.Callback() {
            @Override
    ...
    public static void requestDefrostApp(String packageName, Callback2 callback) {
        ShellClient.command(String.format("pm enable %s", packageName), new ShellClient.Callback() {
            @Override
            public void success(String data) {
                try {
                    JSONObject jsonObject = new JSONObject(data);
    ...
    public static void requestFreezeApp(String packageName, Callback2 callback) {
        ShellClient.command(String.format("pm disable-user %s", packageName), new ShellClient.Callback() {
            @Override
            public void success(String data) {
                try {
                    JSONObject jsonObject = new JSONObject(data);
    ...
    private static void requestRunningProcess(Context context, Callback3 callback) {
        ShellClient.command("ps -ef | grep -vE \"root|system|media|shell|radio|nobody|wifi|gps\"", new ShellClient.Callback() {
            @Override
            public void success(String data) {
                try {
                    JSONObject jsonObject = new JSONObject(data);
